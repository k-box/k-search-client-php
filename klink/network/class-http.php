 ...
 *
 * Standardizes the HTTP requests for Klink Adapter Communication. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Klink HTTP Class for managing HTTP Transports and making HTTP requests.
 *
 * This class is used to consistently make outgoing HTTP requests easy for developers
 * while still being compatible with the many PHP configurations under which
 * Klink Adpter runs.
 *
 * The implementation is highly borrowed from the Wordpress codebase.
 *
 *
 * @package Klink
 * @subpackage Network
 * @since 0.1.0
 */
class KlinkHttp implements INetworkTransport {
	 ...
	 * @since 3.6.0
	 *
	 * @see win_is_writable() and the Wordpress Core
	 *
	...
	* This has the same functionality and prototype of array_column() (PHP 5.5) but also supports objects.
	TODO: move to utils
	*/
	private function list_pluck( $list, $field, $index_key = null ) {
		if ( ! $index_key ) {
			/*
	...
	private function get_temp_dir() {
		...
		if ( defined('WP_TEMP_DIR') )
			return Helpers::trailingslashit(WP_TEMP_DIR);

		if ( $temp )
			return Helpers::trailingslashit( $temp );
	 ...
	 *
	 * }
	 * @return array|KlinkError Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A KlinkError instance upon error.
	 */
	public function request( $url, array $args = array() ) {
		...
		/**
		TODO: validate url for safe protocol, import WP Kses https://core.trac.wordpress.org/browser/tags/4.0/src/wp-includes/kses.php#L0
		*/
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new KlinkError('http_request_failed', Helpers::localize('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new KlinkError( 'http_request_failed', Helpers::localize( 'User has blocked requests through HTTP.' ) );
		...
		if ( $r['stream']  && empty( $r['filename'] ) )
			$r['filename'] = self::get_temp_dir() . basename( $url );
		...
		if ( $r['stream'] ) {
			...
			if ( ! self::is_writable( dirname( $r['filename'] ) ) )
				return new KlinkError( 'http_request_failed', Helpers::localize( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
		...
		if ( ! is_array( $r['headers'] ) ) {
			$processedHeaders = KlinkHttp::processHeaders( $r['headers'], $url );
		...
		Helpers::mbstring_binary_safe_encoding();

		if ( ! isset( $r['headers']['Accept-Encoding'] ) ) {
			if ( $encoding = KlinkHttp_Encoding::accept_encoding( $url, $r ) )
		...
		Helpers::reset_mbstring_encoding();

		if ( Helpers::is_error( $response ) )
			return $response;

		// Append cookies that were used in this request to the response
		if ( ! empty( $r['cookies'] ) ) {
			$cookies_set = self::list_pluck( $response['cookies'], 'name' );
	...
	public function _get_first_available_transport( $args, $url = null ) {
		...
		foreach ( $request_order as $transport ) {
			$class = 'Klink\Network\KlinkHttp_' . $transport;
	 ...
	 * @param string $url URL to Request
	 * @param array $args Request arguments
	 * @return array|KlinkError Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A KlinkError instance upon error
	 */
	private function _dispatch_request( $url, array $args ) {
		static $transports = array();

		$class = $this->_get_first_available_transport( $args, $url );
		if ( !$class )
			return new KlinkError( 'http_failure', Helpers::localize( 'There are no HTTP transports available which can complete the requested request.' ) );
		...
		// if ( is_klink_error( $response ) )
		// 	return $response;

		return $response; //apply_filters( 'http_response', $response, $args, $url );
	}
	...
	public static function processHeaders( $headers, $url = '' ) {
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' == $key )
				$cookies[] = new KlinkHttp_Cookie( $value, $url );
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to KlinkHttp_Cookie instances,
	...
	public static function buildCookieHeader( &$r ) {
		if ( ! empty($r['cookies']) ) {
			// Upgrade any name => value cookie pairs to KlinkHttp_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) )
					$r['cookies'][ $name ] = new KlinkHttp_Cookie( array( 'name' => $name, 'value' => $value ) );
			}
	...
	public function block_request($uri) {
		...
		/**
			TODO: more security with blocked hosts check
		*/
	 ...
	 * @return false|object False if no redirect is present, a WP_HTTP or KlinkError result otherwise.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 )
			return new KlinkError( 'http_request_failed', Helpers::localize('Too many redirects.') );
	...
	public function request($url, $args = array()) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
				return new KlinkError( 'http_request_failed', Helpers::localize( 'The SSL certificate for the host could not be verified.' ) );
		...
		if ( $r['stream'] ) {
			if ( ! WP_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new KlinkError( 'http_request_failed', sprintf( Helpers::localize( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
			...
			while ( ! feof($handle) && $keep_reading ) {
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new KlinkError( 'http_request_failed', Helpers::localize( 'Failed to write request to temporary file.' ) );
				}
...
class KlinkHttp_Curl {
	...
	public function request($url, array $args = array()) {
		...
		curl_exec( $handle );
		$theHeaders = KlinkHttp::processHeaders( $this->headers, $url );
		$theBody = $this->body;
		...
		$response['message'] = Helpers::get_status_header_desc($response['code']);
 ...
 * already a KlinkHttp_Cookie() object, then they are turned into one.
 *
 *
 * @package Klink
 * @subpackage Network
...
class KlinkHttp_Encoding {
	...
	public static function decompress( $compressed, $length = null ) {
		...
		if ( false !== ( $decompressed = KlinkHttp_Encoding::compatible_gzinflate( $compressed ) ) )
			return $decompressed;
	...
	public static function accept_encoding( $url, $args ) {
		$type = array();
		$compression_enabled = KlinkHttp_Encoding::is_available();
