	...
	*/
	...
	public function request( $url, array $args = array() ) {
		...
		*/
		...
		KlinkHelpers::mbstring_binary_safe_encoding();
	...
	public function request($url, array $args = array()) {
		...
		if ( $curl_error || ( 0 == strlen( $theBody ) && empty( $theHeaders['headers'] ) ) ) {
			...
			if ( CURLE_COULDNT_CONNECT /* 7 */ == $curl_error ){
				return new KlinkError( KlinkError::ERROR_CONNECTION_REFUSED, 'Connection refused', KlinkError::ERRORCODE_CONNECTION_REFUSED );
			}
			if ( $curl_error = curl_error( $handle ) ) {
				curl_close( $handle );
				return new KlinkError( KlinkError::ERROR_HTTP_REQUEST_FAILED, $curl_error, KlinkError::ERRORCODE_HTTP_REQUEST_FAILED );
		...
		$response['message'] = KlinkHelpers::get_status_header_desc($response['code']);
 ...
 * @subpackage HTTP
 */
final class KlinkHttp_Encoding {
